		...
		fetch(this.link + '/posts/subject/' + this.props.subjects, {
			...
			.then(posts => {
				...
				for (var i = 0; i < posts.length; i++) {
					if (i <= 40 && posts[i].posterType == "tutor") {// loads the 25 most recent posts
	...
	fetchPostsByUser = () => {
		...
		fetch(this.link + '/posts?name=' + this.state.searchPostInput, {
			...
			.then(posts => {
				...
				for (var i = 0; i < posts.length; i++) {
					if (i <= 40 && posts[i].posterType === "tutor") {// loads the 40 most recent posts
						allPosts.push(posts[i]);
					}
				}
					...
					<BodyWrapper>
						<CenteredSection>
							...
							<ModalFixed show={this.state.expandPostOpen} onClose={() => this.setState({ expandPostOpen: false })}>
								...
								<p style={{marginBottom: '0'}}> Preferred Meeting Location: {this.state.post.location} </p>
								<p style={{marginBottom: '0'}}> {this.state.post.availability} </p>
